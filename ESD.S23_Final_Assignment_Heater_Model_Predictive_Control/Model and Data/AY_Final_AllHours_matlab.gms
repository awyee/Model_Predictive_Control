...
Parameter I(b) Energyloss coefficient
*/
...
Parameter Xmax(b) Max individual electricity draw
*/
*$include Elec_max.inc
*/
*;

Parameter TotalAllowedElecDraw(h) Max aggregator electricity draw
*/
...
T_br_under(h,b)  Breach temperature overage (below min)
T_act(h,b)       Actual Temperature of Building
Totalelec(h)     Total Electricity Consumed by aggregator
...
Equations
         ...
         temp_max(h,b)           Temperature Constraint (max)
         temp_min(h,b)           Temperature Constraint (min)
         temp_calc(h,b)          Temperature Calculation
         elec_req(h,b)           Individual Electricity Constraints due to capacity
         total_consumption(h)    Total Electricity Drawn by aggregator
         elec_draw(h)            Aggregator total electricity limit;
...
Execute_UnloadIdx 'Solution' xL T_ActL T_br_overL T_br_underL TotalelecL;
